...
func (_q *TeamQuery) QueryUser() *UserQuery {
...
func (_q *TeamQuery) QueryRole() *RoleQuery {
...
func (_q *TeamQuery) QueryCredential() *CredentialQuery {
...
func (_q *TeamQuery) QueryChannel() *ChannelQuery {
...
func (_q *TeamQuery) WithUser(opts ...func(*UserQuery)) *TeamQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *TeamQuery) WithRole(opts ...func(*RoleQuery)) *TeamQuery {
	query := (&RoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TeamQuery) WithCredential(opts ...func(*CredentialQuery)) *TeamQuery {
	query := (&CredentialClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TeamQuery) WithChannel(opts ...func(*ChannelQuery)) *TeamQuery {
	query := (&ChannelClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TeamQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Team, error) {
	...
	if query := _q.withUser; query != nil {
		if err := _q.loadUser(ctx, query, nodes,
			func(n *Team) { n.Edges.User = []*User{} },
			func(n *Team, e *User) { n.Edges.User = append(n.Edges.User, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withRole; query != nil {
		if err := _q.loadRole(ctx, query, nodes,
			func(n *Team) { n.Edges.Role = []*Role{} },
			func(n *Team, e *Role) { n.Edges.Role = append(n.Edges.Role, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withCredential; query != nil {
		if err := _q.loadCredential(ctx, query, nodes,
	...
	if query := _q.withChannel; query != nil {
		if err := _q.loadChannel(ctx, query, nodes,
			func(n *Team) { n.Edges.Channel = []*Channel{} },
			func(n *Team, e *Channel) { n.Edges.Channel = append(n.Edges.Channel, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TeamQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Team, init func(*Team), assign func(*Team, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(team.UserColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.team_user
		if fk == nil {
			return fmt.Errorf(`foreign-key "team_user" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "team_user" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *TeamQuery) loadRole(ctx context.Context, query *RoleQuery, nodes []*Team, init func(*Team), assign func(*Team, *Role)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(team.RoleTable)
		s.Join(joinT).On(s.C(role.FieldID), joinT.C(team.RolePrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(team.RolePrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(team.RolePrimaryKey[0]))
...
func (_q *TeamQuery) loadCredential(ctx context.Context, query *CredentialQuery, nodes []*Team, init func(*Team), assign func(*Team, *Credential)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Team)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		fk := n.team_credential
		if fk == nil {
			return fmt.Errorf(`foreign-key "team_credential" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "team_credential" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *TeamQuery) loadChannel(ctx context.Context, query *ChannelQuery, nodes []*Team, init func(*Team), assign func(*Team, *Channel)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Team)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Channel(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(team.ChannelColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.team_channel
		if fk == nil {
			return fmt.Errorf(`foreign-key "team_channel" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "team_channel" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
